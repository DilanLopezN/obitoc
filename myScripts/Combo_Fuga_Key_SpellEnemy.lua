...
addScrollBar("macroDelay", "Config Macro Delay", 10, 1000, 200);
...
STORAGE_DIRECTORY = "" .. MAIN_DIRECTORY .. 'profile_' .. characterName .. '.json';
--STORAGE_DIRECTORY = "" .. MAIN_DIRECTORY .. g_game.getWorldName() .. '.json';
--STORAGE_DIRECTORY = "" .. MAIN_DIRECTORY .. 'profile__' .. currentProfile .. '.json';
...
function canCastFuga()
    for key, value in ipairs(storageProfiles.fugaSpells) do
        if (
            (value.enableLifes and value.lifes > 0 and value.activeCooldown and value.activeCooldown >= os.time()) or
            (not value.enableLifes and value.activeCooldown and value.activeCooldown >= os.time())
            ) then
                return true;
            end
        end
...
function stopToCast()
    ...
    for index, value in ipairs(storageProfiles.fugaSpells) do
        if value.enabled and value.activeCooldown and value.activeCooldown >= os.time() then return false; end
...
comboIcon.onMouseRelease = function (widget, mousePos, mouseButton)
...
function refreshComboList(list, table)
    if table then
        ...
        for index, entry in ipairs(table) do
            ...
            label:setTooltip('Orange Message: ' .. entry.orangeSpell .. ' | On Screen: ' .. entry.onScreen .. ' | Cooldown: ' .. entry.cooldown/1000 .. 's | Distance: ' .. entry.distance)
        end
    end
end
----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
fugaIcon.onMouseRelease = function (widget, mousePos, mouseButton)
  if mouseButton == 2 then 
        fugaInterface:show();
        fugaInterface:raise();
        fugaInterface:focus();
  ...
  if isOn.showInfos then
      for _, value in ipairs(storageProfiles.fugaSpells) do
          if value.selfHealth then
              toShow:show()
              toShow:setText('Inimigos: ' .. getPlayersAttack(false) .. ' | Porcentagem: ' .. calculatePercentage(value.selfHealth) .. ' | Vida: ' .. player:getHealthPercent())
              return;
          end
      end
  else
...
keyIcon.onMouseRelease = function (widget, mousePos, mouseButton)
  if mouseButton == 2 then 
        keyInterface:show();
        keyInterface:raise();
        keyInterface:focus();
...
macro(storage.scrollBars1.macroDelay, function()
  ...
  if isAnySelectedKeyPressed() then return; end
...
local lastFugaTime = 0

macro(50, function()  -- corre cada 50ms, mucho más rápido
  if not fugaIcon.title:isOn() then return; end
  if isInPz() then return; end
  if now - lastFugaTime < 800 then return; end  -- 800ms usando 'now' (ms)
  ...
  for key, value in ipairs(storageProfiles.fugaSpells) do
      if value.enabled and selfHealth <= calculatePercentage(value.selfHealth) then
          if (not value.totalCooldown or value.totalCooldown <= time) and not canCastFuga() then
              stopCombo = now + 200;  -- para el combo antes de la fuga
              say(value.spellCast)
              lastFugaTime = now;
              break;
          end
...
-- code
panelES.onMouseRelease = function (widget, mousePos, mouseButton)
  if mouseButton == 2 then 
        enemyPUI:show();
    end
end
...
enemyPUI.addSpell.onClick = function()
  ...
  if spell == "" or active == "" or cooldown == "" then warn("Algo deu errado!") return end
  table.insert(storage.ES_Spells, {spell = spell, active = active, cooldown = cooldown, enabled = true})
  refreshWidget()
  refreshSpellList()
end
...
onTalk(function(name, level, mode, text, channelId, pos)
  ...
  local target = g_game.getAttackingCreature();
  if g_game.isAttacking() and name == target:getName() then
...
function refreshSpellList()
  ...
  for index, value in pairs(storage.ES_Spells) do
    ...
    --label:setColoredText({ "[", "#8f9ab6", value.spell, "#8f9ab6", "]", "#8f9ab6", ":", "#8f9ab6", " CD: ", "#8f9ab6", value.cooldown,"red", "s", "red", " /", "#000120", " AC: ", "#8f9ab6", value.active, "green" ,"s", "green"})
...
refreshMainWidget()
-- main macro
macro(storage.scrollBars1.macroDelay, refreshTimerWidget)
